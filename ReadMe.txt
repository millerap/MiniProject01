...
In state 0, the fsm prints the 12 bit value read from ain5.

In state 1, the fsm reads an analog potentiometer (ain5). Based on the value across the potentiometer, a percentage is calculated by doing ain/4095 * 100. This percentage is used to set the duty_percent of a PWM output(ehrpwm.1:0), which is connected to an LED.

In state 2, the fsm turns off the PWM.
